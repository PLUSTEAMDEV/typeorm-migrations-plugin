...
export class Import extends BaseEntity {
  ...
  @ManyToOne(() => ImportStatus, (status) => status.imports)
  @JoinColumn([{ name: "status", referencedColumnName: "status" }])
  status: ImportStatus;

  @OneToMany(() => Measure, (measure) => measure.import)
  measures: Measure[];

  @ManyToOne(() => Template, (template) => template.imports, {
    onDelete: "CASCADE",
  })
  @JoinColumn([{ name: "template_id", referencedColumnName: "id" }])
  template: Template;

  @OneToOne(() => ImportFile, (importFile) => importFile.import, {
    onDelete: "CASCADE",
  })
  @JoinColumn([{ name: "import_file_id", referencedColumnName: "id" }])
  importFile: ImportFile;

  @ManyToOne(() => Users, (user) => user.imports, { onDelete: "CASCADE" })
