...
class MigrationGenerator {
     ...
     */
    async createMigrationFile(migrationSqls) {
        try {
            const timestamp = new Date().getTime();
            const content = MigrationUtils_1.MigrationUtils.buildMigrationContent(migrationSqls);
            const fileContent = MigrationUtils_1.MigrationUtils.getTemplate(this.options.migrationName, timestamp, content);
            const filename = timestamp + "-" + this.options.migrationName + ".ts";
            let directory = CONFIG.MIGRATIONS_PATH;
            const filePath = path.join(process.cwd(), directory ? directory + "/" : "", filename);
            await utils_1.createFile(filePath, fileContent);
    ...
    async modifyMigrationFile(fileName, migrationSqls) {
        const fileData = fs
        ...
        fs.renameSync(path.join(CONFIG.MIGRATIONS_PATH, fileName), path.join(CONFIG.MIGRATIONS_PATH, newFileName));
    }
    /**
     ...
     */
    async createOrUpdateMigrationFile(migrationSqls) {
        try {
            const fileName = await MigrationUtils_1.MigrationUtils.getMostRecentMigrationFile();
            if (!fileName) {
     ...
     */
    async generate() {
        const databaseMigrations = await MigrationFactory_1.MigrationFactory.getDatabaseUnitMigrations(this.databaseUnitTypes);
        const migrationSqls = {
            upSqls: [],
            downSqls: [],
        };
